					...
					<div class="panel-body">
						<ul class="list-group">
							...
							@foreach($materiaInNivelSinProfe as $campo)
    							<li class="list-group-item"><em>{{$campo->niveles->nombre_nivel}}</em>. {{$campo->materias->nombre_materia}}</li>
    						@endforeach
    						@endif
   							@endif 
						</ul>
					...
					<div class="panel-body">
						<ul class="list-group">
							...
							@foreach($materiaInNivelConProfe as $campo)
    							<li class="list-group-item"><em>{{$campo->niveles->nombre_nivel}}, {{$campo->materias->nombre_materia}}</em>. {{$campo->empleados->users->name}} {{$campo->empleados->users->name}}. <form method="POST" action="{{ route('editar_profesor')}}">
                                {!! csrf_field() !!}
                                <input type="hidden" name="id" value="{{$campo->id}}">
                                <input type="hidden" name="accion" value="1">
                                <button class="btn btn-primary btn-xs">Borrar profesor</button>
                            </form></li>
    						@endforeach
    						@endif
   							@endif
