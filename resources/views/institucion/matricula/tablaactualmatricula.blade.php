                ...
                @foreach($result as $campo)
                    <tr>
                        ...
                        <td>@if($campo->faltante==0)
                            ...
                            @endif</td>
                        <td>
                            <a class="btn btn-primary btn-xs" href="{{ route('imprimir_matricula') }}/{{ $campo->id }}" target="_blank">Imprimir Tirilla</a>
                            <form method="POST" action="{{ route('editar_matricula')}}">
                                {!! csrf_field() !!}
                                <input type="hidden" name="id" value="{{ $campo->id }}">
                                <input type="hidden" name="accionMatricula" value="2">
                                <button class="btn btn-warning btn-xs">Modificar</button>
                            </form>
                            <form method="POST" action="{{ route('editar_matricula')}}">
                                {!! csrf_field() !!}
                                <input type="hidden" name="id" value="{{ $campo->id }}">
                                <input type="hidden" name="accionMatricula" value="3">
